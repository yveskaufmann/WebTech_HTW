...
/**
 * This class defines the structure of the 'ProductBoard' table.
 *
 *
 *
 ...
 *
 */
class ProductBoardTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Splendr.App.Model.Map.ProductBoardTableMap';
     ...
     */
    const TABLE_NAME = 'ProductBoard';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\Splendr\\App\\Model\\ProductBoard';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Splendr.App.Model.ProductBoard';
     ...
     */
    const COL_ID = 'ProductBoard.id';

    /**
     * the column name for the name field
     */
    const COL_NAME = 'ProductBoard.name';
    ...
    {
        // attributes
        $this->setName('ProductBoard');
        $this->setPhpName('ProductBoard');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\Splendr\\App\\Model\\ProductBoard');
  ...
  array (
    ...
    {
        return $withPrefix ? ProductBoardTableMap::CLASS_DEFAULT : ProductBoardTableMap::OM_CLASS;
    }
     ...
     * @return array           (ProductBoard object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = ProductBoardTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = ProductBoardTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + ProductBoardTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = ProductBoardTableMap::OM_CLASS;
            /** @var ProductBoard $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            ProductBoardTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = ProductBoardTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = ProductBoardTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(ProductBoardTableMap::DATABASE_NAME)->getTable(ProductBoardTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(ProductBoardTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(ProductBoardTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new ProductBoardTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a ProductBoard or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or ProductBoard object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Splendr\App\Model\ProductBoard) { // it's a model object
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                ProductBoardTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return ProductBoardQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a ProductBoard or Criteria object.
     *
     * @param mixed               $criteria Criteria or ProductBoard object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from ProductBoard object
        }

        if ($criteria->containsKey(ProductBoardTableMap::COL_ID) && $criteria->keyContainsValue(ProductBoardTableMap::COL_ID) ) {
...
} // ProductBoardTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
ProductBoardTableMap::buildTableMap();
